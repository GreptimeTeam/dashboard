...
| **筛选 vs Add label** | 不让用户区分 Label 和 Field。两边都是 SQL `WHERE`。筛选键 = 可分组字符串列 + `fieldMap.body`（`=~` 为包含匹配，不做 DISTINCT）。Add label = 可分组列（TAG 或字符串），只排除 `fieldMap.body` / `time` / `severity` 和 JSON 容器，不按列名白名单。Level 仍是 `fieldMap.severity`，单独 select。实现：[`logs/field-map.ts`](../../src/observability/logs/field-map.ts) |
