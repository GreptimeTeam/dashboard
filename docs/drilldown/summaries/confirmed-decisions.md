...
| **Logs filter keys** | 顶栏一个 Filter：label 列（不含 severity）+ 非 label 字符串列。Label = TAG、fieldMap 角色、`labelInclude`、Loki 默认 OTEL resource index-label 列名。非 label 字符串列 `=~` 为包含匹配。没有单独的 Fields 输入框 |
